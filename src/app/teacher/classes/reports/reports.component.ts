...
export class ReportsComponent implements OnInit {
  ...
  getClassStats() {
    this.classes.forEach((clss) => {
      this.CompletedLessonController.getStatsForClass({'id' : clss.id}).subscribe((statsResult) => {
        if(statsResult != null) {
          this.classStats.push({clss: clss, studentStats: statsResult});
      ...
      .subscribe(
        (result: Array<any>) => {
          for(let x = 0; x<result.length; x++){
            lddata.push(result[x].score);
            ldArry.push(result[x].lessonId);
          }
          console.log(result);
        });
  ...
  getAllClasses() {
    ...
    this.ClassController.getClassesWithTeacherName({ 'uName': uName }).subscribe((result) => {
      if (result !== null) {
        let anyResult = result as any;
        anyResult.forEach(inputClass => {
          let newClass: FullClassData = {
            classData: null,
            studentData: null,
          ...
          this.CompletedLessonController.getStatsForClass({ 'id': inputClass.id }).subscribe((statsResult) => {
            if (statsResult != null) {
              let statsResultAny = statsResult;
  ...
  generateStatobject() {
    const uName = localStorage.getItem('logedInUsername');
    this.ClassController.getClassesWithTeacherName({ 'uName': uName }).subscribe((response1) => {
        ...
        this.CompletedLessonController.getStatsForClass({ 'id': classItem.id }).subscribe((response2) => {
          ...
          responseItem2.forEach((studentItem, studentIndex) => {
            if (responseItem1[classIndex].students[studentIndex].username === studentItem.userName) {
              ...
              responseItem1[classIndex].students[studentIndex] = Object.assign(responseItem1[classIndex].students[studentIndex], studentItem, tempStudentActiveBoolean);
            } else {
              ...
              lostItems.forEach((lostItem, lostIndex) => {
                if (responseItem1[classIndex].students[studentIndex].username === lostItem.userName) {
                  ...
                  responseItem1[classIndex].students[studentIndex] = Object.assign(responseItem1[classIndex].students[studentIndex], lostItem, tempStudentActiveBoolean);
          ...
          if (lostItems.length > 0) {
            if (responseItem1[classIndex].students[lostItemStart].username === lostItems[0].userName) {
              ...
              responseItem1[classIndex].students[lostItemStart] = Object.assign(responseItem1[classIndex].students[lostItemStart], lostItems[0], tempStudentActiveBoolean);
  ...
  chartstatsAllClasses(myclasses){
    ...
    myclasses.forEach(element => {
        ...
        for (let i = 0; i < element.students.length; i++) {
          if (element.students[i].activeStudent === true) {
          if (element.students[i].streak > this.highestStreak){
            this.highestStreak = element.students[i].streak;
          }
          this.activeStudents = this.activeStudents + 1;
          this.completedLessons = this.completedLessons + element.students[i].readingLessons;
          this.gradeAvg = this.gradeAvg + element.students[i].comprehension;
          this.wpmAvg = this.wpmAvg + element.students[i].wpm;
          this.timeSpent = this.timeSpent + element.students[i].totalTimeSpentReading;
          this.avereageReadingLevel = this.avereageReadingLevel + element.students[i].reading_level;
          this.lessonsAboveEighty = this.lessonsAboveEighty + element.students[i].readingLessons80Plus;
        }
      }
    });
    this.gradeAvg = this.gradeAvg / this.activeStudents;
